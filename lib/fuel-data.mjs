...
const BRAND_SEARCH_RADIUS_METERS = 150;
...
export async function findStationBrands(stations, cacheDir = defaultCacheDir()) {
  ...
  if (missing.length > 0) {
    // Phase 1: direct match via ref:FR:prix-carburants OSM tag
    const directMatches = await fetchByOsmRef(missing).catch((error) => {
      console.warn(`Lookup OSM ref échoué: ${error.message}`);
      return new Map();
    });

    // Phase 2: proximity fallback only for stations without direct match
    const stillMissing = missing.filter((station) => !directMatches.has(station.id));
    const proximityCandidates =
      stillMissing.length > 0
        ? await fetchOverpassFuelCandidates(stillMissing).catch((error) => {
            console.warn(`Lookup OSM proximité échoué: ${error.message}`);
            return [];
          })
        : [];

    for (const station of missing) {
      const direct = directMatches.get(station.id);
      const match = direct ?? findClosestCandidate(station, proximityCandidates);
      cache[station.id] = match
        ? {
            label: match.label,
            source: "OpenStreetMap",
            distanceMeters: direct ? 0 : Math.round(match.distanceMeters),
            osmId: match.osmId,
...
async function fetchByOsmRef(stations) {
  const idPattern = stations.map((s) => s.id).join("|");
  const query = `[out:json][timeout:20];(
    node["ref:FR:prix-carburants"~"^(${idPattern})$"];
    way["ref:FR:prix-carburants"~"^(${idPattern})$"];
    relation["ref:FR:prix-carburants"~"^(${idPattern})$"];
  );out center tags;`;
  ...
  if (!response.ok) {
    throw new Error(`OSM ref indisponible (${response.status})`);
  }
  ...
  for (const element of data.elements || []) {
    ...
    const refId = tags["ref:FR:prix-carburants"];
    if (!refId) continue;
